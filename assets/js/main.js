...
// Beyond cards: click to reveal + micro animation
gsap.utils.toArray(".beyond-card").forEach((card) => {
  const more = card.querySelector(".beyond-more");
  ...
  card.addEventListener("click", () => {
    ...
    if (open) {
      gsap.fromTo(
        ...
        { maxHeight: 200, opacity: 1, duration: 0.5, ease: "power2.out" }
      );
    } else {
      gsap.to(more, {
        maxHeight: 0,
